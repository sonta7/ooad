...
classSet, departmentSet, teacherSet, classroomSet, courseList, weekTypeSet, timeSet = excel_read(filename='./data/course.xlsx')

conn = sqlite3.connect('db.sqlite3')
cursor = conn.cursor()

sqlInsert_department = "insert into login_department(name, required_selective_credit) values (?, ?)"
sqlInsert_teacher = "insert into login_teachers(code, name, credits, category) values (?,?,?,?)"
sqlInsert_class = "insert into login_class(code, name, credit, category, grade) values (?,?,?,?,?)"
sqlInsert_time = "insert into login_time(week, day, number_choice) values (?,?,?)"
sqlInsert_user = "insert into login_user (name, password,email, sex, c_time,credit_limit, year,department_id) values (?,?,?,?,?,?,?,?)"
sqlInsert_class_department = "insert into login_department_compulsorycourse(department_id, class_id) select A.id, B.id from login_department A, login_class B where A.name=? and B.code=?"

sqlInsert_course = "insert into login_course(teacher, classroom, coursename, capacity, sememster, year, lession_id) select ?,?,?,?,?,?,A.id from login_class A where A.code=?"
sqlInsert_course_time = "insert into login_course_time(course_id, time_id) \
        select A.id, B.id from login_course A , login_class C, login_time B where A.lession_id=C.id and C.name=? and A.coursename=? and B.day=? and B.number_choice=? and B.week=?"
...
def insert_Users():
    ...
    for i in range(4*100):
        insert_User()
    try:
        cursor.execute(sqlInsert_user, ("CC", "CC", "CC@123", 'male', datetime.datetime.now().strftime("%Y-%m-%d %H:%M:?"), 30, 2017, department_id ))
        conn.commit()
    except Exception as e:
        print(e)
        conn.rollback()
...
def insert_preclasses():
    ...
    sql = 'insert into login_prerequisite(course_id, requisiteClass_id, group_number) select A.id, B.id, ? from login_class A, login_class B where A.code=? and B.code=?'
...
def insert_score():
    sql = "insert login_score (lession_id, student_id, score)   \
            select A.id as course_id, B.id as user_id, ? from login_course A, login_user B, login_class C where C.code=? and B.name=? and A.lession_id=C.id and A.year=2017 limit 1"
